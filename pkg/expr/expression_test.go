...
func Test_EvaluateLiteral(t *testing.T) {
	...
	var fns []functions.IFnInfo[interfaces.Fn]
	for _, tc := range tcs {
		t.Run(tc.expr, func(t *testing.T) {
			root := ast.CreateTree(tc.expr, namedValues, fns)
			result, err := evaluator.Evaluate(root, nil, nil, new(evaluator.Option))
...
func Test_EvaluateSimpleFns(t *testing.T) {
	...
	for _, tc := range tcs {
		t.Run(tc.expr, func(t *testing.T) {
			root := ast.CreateTree(tc.expr, namedVals, fns)
			result, err := evaluator.Evaluate(root, nil, nil, new(evaluator.Option))
			assert.NilError(t, err)
			assert.Equal(t, tc.expected, result.Value())
			assert.Equal(t, tc.expectedKind, result.Kind())
...
func Test_EvaluateStatusCheckFns(t *testing.T) {
	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			...
			root := ast.CreateTree(tc.expr, namedVals, fns)
			result, err := evaluator.Evaluate(root, nil, tplCtx, new(evaluator.Option))
			assert.NilError(t, err)
			assert.Equal(t, tc.expected, result.Value())
		})
	}
...
func Test_EvaluateNamedValues(t *testing.T) {
	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			templateCtx := tc.templateCtx()
			root := ast.CreateTree(tc.expr, namedVals, fns)
			result, err := evaluator.Evaluate(root, nil, templateCtx, new(evaluator.Option))
			assert.NilError(t, err)
