...
func newIndexHelper(eCtx interfaces.Context, param interfaces.Node) *indexHelper {
	h := &indexHelper{
		param:  param,
...
func handleFilteredArray(eCtx interfaces.Context, fa filteredArray, i interfaces.Container) any {
...
func handleObject(eCtx interfaces.Context, obj common.Obj, i interfaces.Container) any {
	idx := newIndexHelper(eCtx, i.Parameters()[1])
	if idx.isWildcard() {
		fa := newFilteredArray()
		for _, v := range obj {
...
func handleArray(eCtx interfaces.Context, arr common.Array, i interfaces.Container) any {
	idx := newIndexHelper(eCtx, i.Parameters()[1])
	if idx.isWildcard() {
		fa := newFilteredArray()
