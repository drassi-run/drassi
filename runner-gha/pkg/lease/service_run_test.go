...
func (s *RunServiceTestSuite) TestLease_GetMessage() {
	lease := s.svc.Lease(jobRequest)
	assert.Same(s.T(), jobRequest, lease.GetMessage())
}
...
func (s *RunServiceTestSuite) TestLease_Complete() {
	...
	lease := s.svc.Lease(jobRequest)
	require.NoError(t, lease.Complete(t.Context(), record))
...
func (s *RunServiceTestSuite) TestLease_Complete_Cancel_Renew() {
	...
	require.NoError(t, lease.Complete(ctx, record))
