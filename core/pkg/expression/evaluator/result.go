...
func newEvaluationResultWithTrace(eCtx ast_ifaces.Context, level int, val any, kind expression.ResultKind) *result {
	return newEvaluationResult(eCtx, level, val, kind, false)
}

func newEvaluationResultSkipTrace(eCtx ast_ifaces.Context, level int, val any, kind expression.ResultKind) *result {
	return newEvaluationResult(eCtx, level, val, kind, true)
}

func newEvaluationResult(eCtx ast_ifaces.Context, level int, val any, kind expression.ResultKind, omitTracing bool) *result {
	e := &result{
		kind:        kind,
		value:       val,
		level:       level,
...
func (e *result) Value() any {
	...
	if e.kind == expression.Number && reflect.TypeOf(e.value).Name() == reflect.TypeFor[float64]().Name() {
		f := e.value.(float64)
...
func (e *result) Kind() expression.ResultKind {
	return e.kind
}
...
func (e *result) equal(right expression.Result) bool {
...
func equal(canonicalLeftValue, canonicalRightValue any) bool {
	...
	if lk == rk {
		...
		case expression.Boolean:
			...
			return lB == rB
		case expression.Object, expression.Array:
...
func coerceTypes(canonicalLeftValue, canonicalRightValue any) (leftValue, rightValue any, lk, rk expression.ResultKind) {
	lk = getKind(canonicalLeftValue)
	rk = getKind(canonicalRightValue)
	if lk == rk {
		// Same kind different type
		if lk == expression.Number && reflect.TypeOf(canonicalLeftValue).Name() != reflect.TypeOf(canonicalRightValue).Name() {
	...
	// Boolean|Null, Any
	if lk == expression.Boolean || lk == expression.Null {
		// coerce based on type of right operand
		switch canonicalRightValue.(type) {
		case int:
			canonicalLeftValue = toInt(canonicalLeftValue)
	...
	// Any, Boolean|Null
	if rk == expression.Boolean || rk == expression.Null {
		switch canonicalLeftValue.(type) {
		case int:
			canonicalRightValue = toInt(canonicalRightValue)
			return coerceTypes(canonicalLeftValue, canonicalRightValue)
...
func getKind(canonicalValue any) expression.ResultKind {
...
func (e *result) greaterThan(right expression.Result) bool {
...
func (e *result) greaterThanOrEqual(right expression.Result) bool {
	return e.equal(right) || e.greaterThan(right)
}

func (e *result) lessThan(right expression.Result) bool {
...
func (e *result) lessThanOrEqual(right expression.Result) bool {
	return e.equal(right) || e.lessThan(right)
}

func (e *result) notEqual(right expression.Result) bool {
...
func (e *result) isCollection() (ok bool, collection any) {
	if e.kind == expression.Object || e.kind == expression.Array {
		obj := e.value
...
func isPrimitive(kind expression.ResultKind) bool {
	switch kind {
	case expression.Null, expression.Boolean, expression.Number, expression.String:
		return true
	default:
		return false
	}
