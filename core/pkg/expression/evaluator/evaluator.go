...
func Evaluate(root ast_ifaces.ExprNode, masker secret_masker.Interface, state any, opt *Option) (r expression.Result, err error) {
	defer func() {
		if v := recover(); v != nil {
...
func traceTreeResult(eCtx *context, node ast_ifaces.ExprNode, result any, kind expression.ResultKind) {
	...
	if !strings.EqualFold(realizedExp, traceValue) {
		if kind == expression.Number && realizedExp == fmt.Sprintf("'%s'", traceValue) {
